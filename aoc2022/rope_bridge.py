...
class RopeState:
    def __init__(self, start_position: Optional[Position] = None, num_knots: int = 2):
        if start_position is None:
            start_position = Position(0, 0)
        self.start_position = start_position
        self.knots: List[Position] = [start_position] * num_knots
    ...
    def head_position(self, new_pos: Position):
        self.knots[-1] = new_pos
        self.head_history.append(new_pos)
    ...
    def tail_position(self, new_pos: Position):
        self.knots[0] = new_pos
        self.tail_history.append(new_pos)

    def move(self, direction: Direction):
        self.head_position = self.head_position.move(direction)
        for i in range(len(self.knots) - 2, -1, -1):
            # loop through the non-head knots, one at a time
            next_knot = self.knots[i+1]
            knot = self.knots[i]
            row_delta = next_knot.row - knot.row
            col_delta = next_knot.col - knot.col
            ...
            if abs(row_delta) > 1 or abs(col_delta) > 1:
                ...
                new_position = Position(row=knot.row + row_delta, col=knot.col + col_delta)
                if i == 0:
                ...
                match Position(row=row, col=col):
                    ...
                    case other:
                        for i, k in enumerate(self.knots[1:-1]):
                            if k == other:
                                s.append(str(len(self.knots) - i - 2))
                            break
                        else:
                            s.append(".")
            s.append("\n")

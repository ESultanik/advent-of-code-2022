...
class SNAFU(int):
    @staticmethod
    def to_string(snafu: int) -> str:
        if snafu < 0:
            raise NotImplementedError()
        original = snafu
        num_digits = 0
        # the maximum value that can be represented by an n-digit SNAFU number is
        # sum((5**i)*2 for i in range(n))
        # which equals
        # (5**n - 1) // 2
        max_value_with_digits: list[int] = []
        while True:
            max_value = (5**num_digits - 1) // 2
            max_value_with_digits.append(max_value)
            if max_value >= snafu:
                break
            num_digits += 1
        digits = [0] * num_digits
        ...
        for digit in range(num_digits):
            remaining_digits = num_digits - digit - 1
            digit_base_value = 5**remaining_digits
            remaining_max_value = max_value_with_digits[remaining_digits]
            assert -digit_base_value * 2 - remaining_max_value <= snafu <= digit_base_value * 2 + remaining_max_value
            # next_digit_max_value = max((5**(num_digits - digit - 2)) * 2, 0)
            if snafu > remaining_max_value:
                # we need to increase digit's value
                if snafu > digit_base_value + remaining_max_value:
                    digits[digit] = 2
                else:
                    digits[digit] = 1
            elif snafu < -remaining_max_value:
                # we need to decrease digit's value
                if snafu < -digit_base_value - remaining_max_value:
                    digits[digit] = -2
                else:
                    digits[digit] = -1
            snafu -= digits[digit] * digit_base_value
        assert snafu == 0
        result = "".join(("=-012"[digit + 2] for digit in digits))
        assert SNAFU.parse(result) == original
        return result

    def __str__(self):
        return SNAFU.to_string(self)

    @classmethod
    def parse(cls, snafu: str) -> "SNAFU":
        exponent = 1
        snafu = snafu.strip()
        value = 0
        while snafu:
...
@challenge(day=25)
def snafu_sum(path: Path) -> str:
    with open(path, "r") as f:
        s = sum(
            SNAFU.parse(line)
            for line in f
        )
        return SNAFU.to_string(s)
